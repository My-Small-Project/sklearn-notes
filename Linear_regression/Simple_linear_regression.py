...
# print(X.shape) (5,1)
# print(model.predict(X).shape) (5,)

test_pizza = np.array([[12]])  # 需要预测的x
print("the predicted price:", model.predict(test_pizza)[0])  # 输出价格,原来为[13]，用[0]表示输出这个值
...
r_squared = model.score(X_test, y_test)  # 求R方得分，用来衡量数据和回归线的贴近程度
print(r_squared)
